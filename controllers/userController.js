  ...
  try {
    ...
    const mailOptions = {
      ...
      html:
        "<p>Hi " +
        name +
        ',Please Click Here to <a href="www.abinbinu.com/verify?id=' +
        user_id +
...
const insertUser = async (req, res) => {
  try {
    ...
    if (mobileExists) {
      return res.render("users/signup", {
        message:
          "Mobile already exists. Please enter a different Mobile Number.",
      });
    }
    ...
    if (userData) {
      ...
      res.render("users/emailVerificationNotation", {
        message:
          "You signup is sucessfull pls Re-enter Your Mobile for verification...",
      });
    } else {
...
const forgetVerify = async (req, res) => {
  try {
    ...
    if (userData) {
      ...
      res.render("users/forget", {
        message: "User email is incorrect Or Please enter a valid mail",
      });
    }
  } catch (error) {
    console.log(error.message);
  ...
  try {
    ...
    const mailOptions = {
      ...
      html:
        "<p>Hi " +
        name +
        ',Please<a href="www.abinbinu.com/forget-password?token=' +
        token +
...
const editUser = async (req, res) => {
  try {
    if (!req.body.name || !req.body.email || !req.body.mobile) {
      return res
        .status(400)
        .json({ error: "Name, email, and mobile are required fields." });
    }
  ...
  try {
    ...
    if (defaultAddress === null) {
      ...
      const eligibleCoupon =
        await couponHelper.checkCurrentCouponValidityStatus(
          userId,
          finalAmount
        );
...
const generateWalletRechargeOrder = async (req, res) => {
  try {
    ...
    const razorpayResponse = await userHelpers.generateRazorpayForWallet(
      userId,
      total
    );
    ...
    userHelpers
      ...
      .then(() => {
        const razorpayAmount = parseInt(
          req.body["serverOrderDetails[razorpayResponse][amount]"]
        );
        const amount = parseInt(razorpayAmount / 100);
        userHelpers
          .rechargeUpdateWallet(
            req.body["serverOrderDetails[razorpayResponse][receipt]"],
            amount
          )
          .then(() => {
...
const placeOrder = async (req, res) => {
  try {
    ...
    if (productsOrdered) {
      ...
      const orderId = await productHelper.placingOrder(
        userId,
        orderDetails,
        productsOrdered,
        totalOrderValue
      ...
      if (req.body["paymentMethod"] === "COD") {
        ...
        const razorpayOrderDetails = await productHelper.generateRazorpayOrder(
          orderId,
          totalOrderValue
        );
        const user = await User.findById({ _id: userId }).lean();
        ...
        if (walletBalance.walletAmount >= totalOrderValue) {
          await productHelper.placingOrder(
            userId,
            orderDetails,
            productsOrdered,
            totalOrderValue
...
const returnReason = async (req, res) => {
  try {
    ...
    const updatedOrder = await Order.findByIdAndUpdate(
      ...
      {
        $set: {
          orderStatus: "Return Procedure",
          cancellationStatus: "Return Procedure",
...
const downloadInvoice = async (req, res) => {
  try {
    ...
    order.products.forEach((product, index) => {
      ...
      pdfDoc.text(
        `Product Purchased:${index + 1}: ${product.productId.productname}`
      );
